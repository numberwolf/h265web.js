...
if (!ENVIRONMENT_IS_PTHREAD) {
	var STACK_BASE = 1389264,
	STACK_MAX = 6632144,
	DYNAMIC_BASE = 6632144,
	DYNAMICTOP_PTR = 1388240;
...
if (!ENVIRONMENT_IS_PTHREAD) addOnPreRun(function() {
	if (typeof SharedArrayBuffer !== "undefined") {
		...
		PThread.allocateUnusedWorkers(10,
		function() {
...
var wasmBinaryFile = "missile-v20220507.wasm";
...
if (!ENVIRONMENT_IS_PTHREAD) {
	memoryInitializer = "missile-v20220507.html.mem"
}
var tempDoublePtr;
if (!ENVIRONMENT_IS_PTHREAD) tempDoublePtr = 1389248;
...
else PthreadWorkerInit.__main_thread_futex_wait_address = __main_thread_futex_wait_address = 1389232;
function _emscripten_futex_wake(addr, count) {
	...
	if (mainThreadWaitAddress == addr) {
		...
		if (loadedAddr == mainThreadWaitAddress) {--count;
			mainThreadWoken = 1;
			if (count <= 0) return 1
		}
	}
...
var PThread = {
	...
	initMainThreadBlock: function() {
		...
		PThread.mainThreadBlock = 1388448;
	...
	attachListenerToWorkers: function(workers, onFinishedLoading) {
		...
		for (var i = 0; i < numWorkers; ++i) {
			var worker = workers[i]; (function(worker) {
	...
	createNewWorkers: function(numWorkers) {
		if (typeof SharedArrayBuffer === "undefined") return [];
		var pthreadMainJs = "missile-v20220507.worker.js";
...
else PthreadWorkerInit._fetch_work_queue = _fetch_work_queue = 1388432;
...
var Fetch = {
	...
	staticInit: function() {
		...
		if (isMainThread) {
			...
			var fetchJs = locateFile("missile-v20220507.fetch.js");
...
else PthreadWorkerInit.___tm_timezone = ___tm_timezone = (stringToUTF8("GMT", 1388336, 4), 1388336);
...
function checkUnflushedContent() {
	...
	try {
		...
		if (flush) flush(0); ["stdout", "stderr"].forEach(function(name) {
