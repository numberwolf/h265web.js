...
class CWsLiveCoreModule { // export default 
    ...
    start(url265) {
            ...
            self.onmessage = (event) => {
                ...
                switch (cmd) {
                    case 'start':
                        ...
                        let WSURL = body.data;
                        // ws://127.0.0.1/live/0/test.flv
                        WSocket = new WebSocket(WSURL);
                        // var ws = new WebSocket("ws://www.panll.top:1380/123/1231.flv");
                        ...
                        WSocket.onmessage = function(wsevent) {
                            ...
                            if (count > 1000) {
                                WSocket.close();
                                console.log("WSocket.close()", count);
                                // downloadBlob(totalData, 'wstest1.flv', 'application/octet-stream');
                                postMessage({
                                    cmd: 'fetch-close',
                                    data: '',
                                    msg: 'fetch-close'
        ...
        this._ptr_naluCallback    = Module.addFunction(this._callbackNALU.bind(this), "viiiiidi");
...
} // end class CWsLiveCoreModule

exports.CWsLiveCore = CWsLiveCoreModule;
